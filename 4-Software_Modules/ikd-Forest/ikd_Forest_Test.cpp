...
int main(int argc, char** argv){
    ...
    while (counter < Test_Time){           
        ...
        for (int k=0;k<Search_Counter;k++){
            ...
            query[0][1] = target.y;
            query[0][2] = target.z;
            flann::Matrix<int> indices(new int[query.rows * Nearest_Num], query.rows, Nearest_Num);
            flann::Matrix<float> dists(new float[query.rows * Nearest_Num], query.rows, Nearest_Num);
            t1 = chrono::high_resolution_clock::now();
            index.knnSearch(query, indices, dists, Nearest_Num, flann::SearchParams(flann::FLANN_CHECKS_UNLIMITED,0,true));
            t2 = chrono::high_resolution_clock::now();
            ...
            for (int i=0;i<search_result.size();i++){
                ...
                printf("No.%d Point is (%0.3f,%0.3f,%0.3f)\n",i, search_result[i].x, search_result[i].y, search_result[i].z);
                PointType tmp_point;
