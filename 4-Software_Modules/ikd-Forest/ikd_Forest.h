...
struct PointType_CMP{
    PointType point;
    float dist;
    PointType_CMP (PointType p, float d, double timestamp, double update_counter){
        this->point = p;
        this->point.intensity = timestamp; //add by cn for return timestamp
        this->point.curvature = update_counter;
        this->dist = d;
    };
